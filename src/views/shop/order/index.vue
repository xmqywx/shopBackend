...
<!--      </div>-->
      ...
      <!--表格渲染-->
      <el-table v-loading="loading" :data="data" size="small" style="width: 100%;" @expand-change="expandChange">
        <el-table-column prop="cartInfoList" width="50" type="expand" label="">
          <template slot-scope="scope">
            <div style="padding:0 40px;">
            <el-table :data="scope.row.cartInfoList" size="small" style="width: 100%;background: #333;">

              <el-table-column prop="cartId" label="商品编号">
                <template slot-scope="pro">
                  {{ pro.row.cartInfoMap.productInfo.ertId }}
                </template>
              </el-table-column>
              <el-table-column prop="cartId" label="商品名称">
                <template slot-scope="pro">
                  {{ pro.row.cartInfoMap.productInfo.store_name }}
                </template>
              </el-table-column>
              <el-table-column prop="cartId" label="规格">
                <template slot-scope="pro">
                  {{ pro.row.cartInfoMap.productInfo.erpGoodsspec }}
                </template>
              </el-table-column>
              <el-table-column prop="cartId" label="生产厂家">
                <template slot-scope="pro">
                  {{ pro.row.cartInfoMap.productInfo.erpManufacturer }}
                </template>
              </el-table-column>
              <el-table-column prop="cartId" label="单位">
                <template slot-scope="pro">
                  {{ pro.row.cartInfoMap.productInfo.erpUnit }}
                </template>
              </el-table-column>

              <el-table-column prop="cartId" label="购买数量">
                <template slot-scope="pro">
                  {{ pro.row.cartInfoMap.cartNum }}
                </template>
              </el-table-column>
              <el-table-column prop="cartId" label="单价">
                <template slot-scope="pro">
                  {{ pro.row.cartInfoMap.productInfo[scope.row.userDTO.price] || pro.row.cartInfoMap.truePrice}}
                </template>
              </el-table-column>
              <el-table-column prop="cartId" label="金额">
                <template slot-scope="pro">
                  {{ (pro.row.cartInfoMap.productInfo[scope.row.userDTO.price] || pro.row.cartInfoMap.truePrice) * pro.row.cartInfoMap.cartNum }}
                </template>
              </el-table-column>

              <el-table-column prop="cartId" label="规划分类">
                <template slot-scope="pro">
                  {{ pro.row.cartInfoMap.productInfo.erpPlancate }}
                </template>
              </el-table-column>
              <el-table-column prop="cartId" label="批准文号">
                <template slot-scope="pro">
                  {{ pro.row.cartInfoMap.productInfo.erpApprovalno }}
                </template>
              </el-table-column>
            ...
            :key="index">
                ...
                <span> | ￥{{ item.cartInfoMap.productInfo[scope.row.userDTO.price] }}×{{ item.cartInfoMap.cartNum }}</span>
            </div> -->
          </template>
        </el-table-column>
        ...
        <el-table-column prop="realName" label="客户名称" >
          <template slot-scope="scope">
            <span>{{ scope.row.userDTO.realName }}</span>
          </template>
        </el-table-column>
        <el-table-column prop="realName" label="客户地址" >
          <template slot-scope="scope">
            <span>{{ scope.row.userDTO.addres }}</span>
          </template>
        </el-table-column>
  ...
  export default {
    ...
    data() {
      return {
        ...
        queryTypeOptions: [
          ...
          // { key: 'username', display_name: '客户名称' },
          { key: 'userPhone', display_name: '联系电话' },
          { key: 'storeName', display_name: '商品信息' },
        ],
        statusOptions: [
          { value: '0', label: '未支付' },
          { value: '1', label: '未发货' },
    ...
    methods: {
      ...
      expandChange(row, type) {
        ...
        row.cartInfoList.forEach(async (item, index) =>{
          const res = await getInfo(item.cartInfoMap.productId);

          row.cartInfoList.splice(index, 1, {
            ...item,
            cartInfoMap: {
              ...item.cartInfoMap,
